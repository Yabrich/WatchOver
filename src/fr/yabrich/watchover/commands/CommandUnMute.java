...
public class CommandUnMute implements CommandExecutor {
	...
	public CommandUnMute(Main main) {
		this.main = main;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String msg, String[] args) {
		if(sender instanceof Player) {
			...
			if(args.length != 1) {
				...
				player.sendMessage("§cUsage : /unmute [joueur]");
			...
			for(String sanction : sanctionsJoueur) {
				if(sanction.contains("Type : Mute")) {
					...
					if(today.isBefore(date_fin_formated)) {
						...
						player.sendMessage("§4[§6WatchOver§4] §3Vous venez de redonner la parole à §e"+nickname);
						
						//Message targetPlayer si joueur connecté
						
						if(Bukkit.getPlayer(args[0]) != null) {
							Player targetPlayer = Bukkit.getPlayer(args[0]);
							
							targetPlayer.sendMessage("§2Un modérateur vient de vous redonner la parole.");
						}
