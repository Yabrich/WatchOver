	...
	public boolean onCommand(CommandSender sender, Command cmd, String msg, String[] args) {
		if(sender instanceof Player) {
			...
			if(args.length < 3) {
				...
				player.sendMessage("§cUsage : /mute [joueur] [temps] [raison]");
			...
			//Check si pas déjà mute actuellement
			for(String sanction : sanctionsJoueur) {
				if(sanction.contains("Type : Mute")) {
					...
					if(today.isBefore(date_fin_formated)) {
						player.sendMessage("§cErreur : Ce joueur est déjà mute actuellement !");
						player.sendMessage("§cMerci d'unmute puis remute ensuite afin de changer la sanction");
						return true;
					}
				}
			}
			...
			String sanction = "ID : "+id_punishment+" | Type : Mute | Raison : "+ raison + "| Moderateur : "+player.getName()+" | Date_Debut : "+date_debut+" | Date_Fin : "+date_fin_format;
			...
			//Message utilisateur
			player.sendMessage("§4[§6WatchOver§4] §3Vous avez réduit au silence §e"+nickname+" §3pour la raison §7: §6"+raison);
			player.sendMessage("§4[§6WatchOver§4] §3La sanction expirera le §6"+date_fin_format+" §2[§aID:"+id_punishment+"§2]");
			
			Bukkit.broadcastMessage("§cUn joueur a été réduit au silence par un modérateur.");
			
			if(Bukkit.getPlayer(nickname) != null) {
				Player targetPlayer = Bukkit.getPlayer(nickname);
				
				targetPlayer.sendMessage("§6§kHHHH§r§6 -§4§l Réduction au silence§r§6 - §6§kHHHH");
				targetPlayer.sendMessage("§6Vous venez d'être réduit au silence par un modérateur pour la raison suivante : §e"+raison);
